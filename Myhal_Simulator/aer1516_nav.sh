...
# Wait until the ground_truth/pose topic is published
until [[ -n "$gt_pose_msg" ]]
do 
    sleep 0.5
...
if [ "$MAPPING" = "0" ] ; then
    ...
    loc_launch="point_slam simu_ptslam.launch filter:=$FILTER gt_pos_x:=$gt_pos_x gt_pos_y:=$gt_pos_y gt_pos_z:=$gt_pos_z gt_ori_w:=$gt_ori_w gt_ori_x:=$gt_ori_x gt_ori_y:=$gt_ori_y gt_ori_z:=$gt_ori_z"
fi
